...
for ( h in h_list ){
  ...
  saveRDS(p, paste0("pvals_var_cusum_H0_wbs_T", n, "_h", h, ".rds"))
}
...
saveRDS(p, paste0("pvals_var_cusum_H0_wbs_T", n, "_hNA.rds"))
...
saveRDS(p, paste0("pvals_var_cusum_H1_wbs_T", n, "_K1.rds"))
...
N <- 1000
n <- 200
p10 <- readRDS(paste0("pvals_var_cusum_H0_wbs_T", n, "_h10.rds"))
p20 <- readRDS(paste0("pvals_var_cusum_H0_wbs_T", n, "_h20.rds"))
p50 <- readRDS(paste0("pvals_var_cusum_H0_wbs_T", n, "_h50.rds"))
pNA <- readRDS(paste0("pvals_var_cusum_H0_wbs_T", n, "_hNA.rds"))

dat_H0 <- data.frame(z=rep(seq(0, 1, length.out=N)), p=c(sort(p10), sort(p20), sort(p50), sort(pNA)),
                     h=as.factor(rep(c("10", "20", "50", "NA"), each=N)))

p <- readRDS(paste0("pvals_var_cusum_H1_wbs_T", n, "_K1.rds"))
p[p > 1] <- 1
h_list <- dimnames(p)[[2]]
