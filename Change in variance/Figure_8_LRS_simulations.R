# Simulating under H0 and H1, using likelihood ratio to estimate changepoints + Gaussian processes to estimate S
...
while ( iter <= NN ){
  ...
  if ( length(results$changepoints) >= 1 & results$changepoints[1] >= h & results$changepoints[1] <= length(x) - h ){
    for ( i_N in 1:length(N_list) ){
      N_sample <- N_list[i_N]
      for ( i_frac in 1:length(fracs) ){
        frac <- fracs[i_frac]
        p_hat[iter, (i_N - 1) * length(fracs) + i_frac] <- calculate_pvals_var(results, h, N_sample, frac, num_pvals=1, cp_bound=FALSE)$p
      }
    }
    print(iter)
    iter <- iter + 1
...
saveRDS(p_hat, paste0("pvalues_gp_lrs_H0_T", n, "_h", h, "_thresh", threshold, ".rds"))
...
while ( iter <= NN ){
  ...
  results <- find_changepoints(x, "bs", list(threshold=threshold, loss="lrs"), model="var")
  if ( length(results$changepoints) >= 1 & results$changepoints[1] >= h & results$changepoints[1] <= length(x) - h ){
...
saveRDS(p_hat, paste0("pvalues_gp_lrs_H1_T", n, "_h", h, "_thresh", threshold, "_K3_fixed_changes.rds"))
...
saveRDS(p_hat, paste0("pvalues_gp_lrs_H1_T", n, "_h", h, "_thresh", threshold, "_K3_random_changes.rds"))

#############################################################################################################################

# Plots

p1 <- readRDS("pvalues_gp_lrs_H0_T200_h20_thresh2.rds")
fracs <- c(0.2, 0.5, 0.8, 1)
N_list <- c(50, 100, 200)
NN <- nrow(p1)
...
p2 <- readRDS("pvalues_gp_lrs_H1_T200_h20_thresh6_K3_fixed_changes.rds")
fracs <- c(0.2, 0.5, 0.8, 1)
N_list <- c(50, 100, 200)
NN <- nrow(p2)
p_all <- z_all <- method_all <- N_all <- numeric(0)
...
p3 <- readRDS("pvalues_gp_lrs_H1_T200_h20_thresh6_K3_random_changes.rds")
fracs <- c(0.2, 0.5, 0.8, 1)
N_list <- c(50, 100, 200)
NN <- nrow(p3)
p_all <- z_all <- method_all <- N_all <- numeric(0)
